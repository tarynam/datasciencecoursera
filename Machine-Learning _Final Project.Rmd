...
stopCluster(cluster)
registerDoSEQ()
kable(data.frame(fit_knn$resample), caption = "K-Nearest Neighbour Resampling")
```
...
stopCluster(cluster)
registerDoSEQ()
kable(data.frame(fit_svm$resample), caption = "Support Vector Machine Resampling")
```
...
stopCluster(cluster)
registerDoSEQ()
kable(data.frame(fit_nnet$resample), caption = "NNET Resampling")
```
...
stopCluster(cluster)
registerDoSEQ()
kable(data.frame(fit_rf$resample), caption = "Random Forest Resampling")
```
...
stopCluster(cluster)
registerDoSEQ()
kable(data.frame(fit_gbm$resample), caption = "Gradient Boosted Resampling")
```

##Evaluating Models
The final model selected by caret using 10-fold cross validation for each model type is reported below along with the final model's accuracy, kappa statistic, and confusion matrix.
```{r evaluation}
library(knitr)
library(caret)
a<-kable(confusionMatrix.train(fit_knn)$table, caption = "K-Nearest Neighbours")
b<-kable(confusionMatrix.train(fit_svm)$table, caption = "Support Vector Machine")
c<-kable(confusionMatrix.train(fit_nnet)$table, caption = "NNET")
d<-kable(confusionMatrix.train(fit_rf)$table, caption = "Random Forest")
e<-kable(confusionMatrix.train(fit_gbm)$table, caption = "Gradient Boosted Machine")
```

```{r print tables, echo = FALSE, eval=TRUE}
a
```
  
Using caret, a K-nearest Neighbors model was built with k=5. This model was `r max(fit_knn$results$Accuracy)` accurate and and had a kappa statistic of `r max(fit_knn$results$Kappa)` in the resampled training data.

```{r, echo = FALSE, eval=TRUE}
b
```
  
Using caret, a SVM model was built holding the tuning parameter "C" constant at a value of 1. This model was `r max(fit_svm$results$Accuracy)` accurate and and had a kappa statistic of `r max(fit_svm$results$Kappa)` in the resampled training data.

```{r, echo = FALSE, eval=TRUE}
c
```
  
Using caret, a Neural Netowrk model was built with 1 layer, 1 neuron and weight decay = 0.1. This model was `r max(fit_nnet$results$Accuracy)` accurate and and had a kappa statistic of `r max(fit_nnet$results$Kappa)` in the resampled training data. This model clearly had probblems as it predicted all data as class "A"

```{r, echo = FALSE, eval=TRUE}
d
```
  
Using caret, a Random Forest model was built with 28 trees. This model was `r max(fit_rf$results$Accuracy)` accurate and and had a kappa statistic of `r max(fit_rf$results$Kappa)` in the resampled training data. 

```{r, echo = FALSE, eval=TRUE}
e
```

Using caret, a Gradient Boosted Machine model was built with 150 trees of depth 3 and a learning rate of 0.1. This model was `r max(fit_gbm$results$Accuracy)` accurate and and had a kappa statistic of `r max(fit_gbm$results$Kappa)` in the resampled training data. 

###Final Decision
The best model for this data, based on both the accuracy, is the Random Forest model and this will be used on the test data. I think the out of sample error will be low due to the use of cross validation. However I still expect the accuracy on new data to be lower than the in sample accuracy of `r max(fit_rf$results$Accuracy)`.
